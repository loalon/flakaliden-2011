...
## according to the previous analysis W25-F3 is not good quality and is removed
## count table is subset to control treatment only

count.table <- count.table[,order(colnames(count.table))]
count.table <- count.table[,!colnames(count.table) %in% c("W25-F3")]
count.table <- count.table[,grep("F",colnames(count.table))]

### ==============================
## 18.6 % of the genes are not expressed from a total of 70,736 genes
### ==============================
...
conditions <- sub("-[F][1-6]m?","",colnames(count.table))
...
### =============================
## Cutoff 2 seems to be sufficient enough for getting an equal variance
###============================
vsd2 <- vsd[rowMeans(assay(vsd)) > 2,]
...
## Here: 57575 out of 70736 genes are expressed in all samples
### ==============================
...
barplot(table(df$sample.expressed)[0:55])
title(ylab = "number of genes", xlab = "number of samples (0-56)", font.lab = 1)

barplot(table(df$sample.expressed)[4:52])
...
par(mar=c(4.1,4.1,1.1,0.1))
boxplot(split(df$mean.expression,df$sample.expressed)[0:54])
title(ylab = "Mean expression", xlab = "Number of samples with expressed genes (0:54)", font.lab = 1)
...
abline(v=match("55",df[order(df$mean.expression),"sample.expressed"]))
...
df[order(df$mean.expression),][match("55",df[order(df$mean.expression),"sample.expressed"]),]
...
pos <- match("55",df[order(df$mean.expression),"sample.expressed"])
