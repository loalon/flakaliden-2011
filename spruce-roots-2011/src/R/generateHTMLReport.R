...
### =============================
## select different cutoffs from 1-3 and plot the mean against the variance again.
## Cutoff 2 seems to be sufficient enough for getting an equal variance
###============================
sel1 <- rowMeans(assay(vsd)) > 1
meanSdPlot(assay(vsd)[rowMeans(assay(vsd)) > 1,])
title(main="Mean vs Variance - cutoff:1")

sel2 <- rowMeans(assay(vsd)) > 2
meanSdPlot(assay(vsd)[rowMeans(assay(vsd)) > 2,])
title(main="Mean vs Variance - cutoff:2")

sel3 <- rowMeans(assay(vsd)) > 3
meanSdPlot(assay(vsd)[rowMeans(assay(vsd)) > 3,])
title(main="Mean vs Variance - cutoff:3")
...
## no diferentiation between control and fertilised
### ==============================
scatterplot3d(pc$x[,1],
              ...
              color=cols,pch=pchs)
### ==============================
## plotted by treatment - control (C) and fertilised (F)
### ==============================
scatterplot3d(pc$x[,1],
              pc$x[,2],
              pc$x[,3],
...
title(ylab = "Mean expression", xlab = "Number of samples with expressed genes (0:110)", font.lab = 1)
