...
for f in `find $in -name "2_120823_AD16FAACXX_229_197_index27_*trimmomatic_[1,2].fq.gz" -type f`; do echo "${f//_trimmomatic_[1,2].fq.gz/}" ; done | sort | uniq | while read line;
do 
fnam=`basename $line`
sbatch -A b2010042 --mail-user $mail -e $out/$fnam.err -o $out/$fnam.out -J smr-$fnam ../../../pipeline/runSortmerna.sh $out $tmp ${line}_trimmomatic_1.fq.gz ${line}_trimmomatic_2.fq.gz
done
