---
title: "Fungi 2011 project - dbCAN analysis"
...
# Introduction
This file contains the script and results for the dbCAN analysis done to the
fungi 2011 data. 
...
#source("~/Git/Rtoolbox/src/plot3Dvector.R")
source("~/Git/Rtoolbox/src/plotVectorPCA.R")
source("~/Git/Rtoolbox/src/plot3Dpca.R")
...
Read metadata
```{r}
meta <- read.table(file.path(projectFolder, "data", "meta.tsv"), header = TRUE, sep = '\t', comment.char = "", stringsAsFactors = TRUE)
...
data <- read.table(file.path(projectFolder, "data", "dbCAN.raw.tsv"), header = T, sep='\t', comment.char = "", quote="", row.names = 1)
...
Remove 0 expression rows (if any)
```{r}
...
```{r}
write.table(rownames(assay(ddsMatrix)), file=paste0(deFolder, "background_dbCAN_fungi_only.txt"), sep='\t', row.names = F, col.names=F, quote = F)
```
...
```{r}
plot3Dpca(vsd.QA.t, meta,c("Treatment"))
```
...
The first 5 components explain 59 % of the variance
```{r}
...
save(combinedData, file=paste0(deFolder,"combinedVsdData_dbCAN.RData"))
write.table(combinedData, file =paste0(deFolder,"fungi2011Combined_dbCAN.tsv"), sep='\t', row.names=FALSE, col.names=FALSE)
writeLines(colnames(combinedData), con=paste0(deFolder,"fungi2011CombinedNames_dbCAN.txt"))
```
...
vsdSplit <- varianceStabilizingTransformation(ddsSplit, blind=F)
...
save(controlData, file=paste0(deFolder,"controlVsdData_dbCAN.RData"))
write.table(controlData, file = paste0(deFolder,"fungi2011Control_dbCAN.tsv"), sep='\t', row.names=FALSE, col.names=FALSE)
writeLines(colnames(controlData), con=paste0(deFolder,"fungi2011ControlNames_dbCAN.txt"))

save(fertilisedData, file=paste0(deFolder,"fertilisedVsdData_dbCAN.RData"))
write.table(fertilisedData, file = paste0(deFolder,"fungi2011Fertilised_dbCAN.tsv"), sep='\t', row.names=FALSE, col.names=FALSE)
writeLines(colnames(fertilisedData), con=paste0(deFolder,"fungi2011FertilisedNames_dbCAN.txt"))
