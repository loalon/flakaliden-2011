...
```{r controlViolin}
write.table(controlDEG, file=here("results",paste0("controlDEG_",Sys.Date(),".tsv") ) , quote = F, sep='\t', row.names = F)
...
write.table(mostDegraded, file=here("results",paste0("mostDegraded_",Sys.Date(),".tsv") ) , quote = F, sep='\t', row.names = F)
...
None <- sapply(c("go","pfam","mapman","kegg"), function(x){
  if(!is.null(spruceMulti$enr1[[x]])) {
    png(here("results", paste0("treemap_", x, Sys.Date(), ".png")), width = 1920, heigh=1080)
  }
  
})
plotEnrichedTreemap(spruceMulti$enr1, enrichment = "go", namespace = "BP")
plotEnrichedTreemap(fungiMulti$enr1, enrichment = "ko_pathway", namespace = "BP")
