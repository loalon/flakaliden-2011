...
The first 5 components explain 57 % of the variance
```{r}
...
```{r eval=FALSE}
combinedFvsC.res <- mapply(getRes, fertilisedVector, controlVector,
                            MoreArgs = list(localDDS=dds, group="group"))
```
...
names(combinedFvsC.res) <- c("Fertilised_Early_JunevsControl_Early_June", 
...
combinedFvsC.res.filter <- lapply(combinedFvsC.res, filterDE)
...
lapply(combinedFvsC.res.filter, function(x) print(all(abs(x$log2FoldChange)>0.5 & x$padj<0.01)) )
...
```{r}
plotUpAndDown(combinedFvsC.res.filter)
```
...
de4network(combinedFvsC.res, file.path(deFolder,"deCombinedFvsC.tsv"))
