...
data <- read.table(file.path(dataFolder, "kingdom.Fungi.kos.raw.tsv"), header = T, sep='\t', comment.char = "", quote="", row.names = 1)
...
```{r}
write.table(rownames(assay(ddsMatrix)), file=file.path(deFolder, "background.txt"), sep='\t', row.names = F, col.names=F, quote = F)
```
...
```{r plotGroupPCA}
geneNumber <- length(rownames(assay(vsd.QA))) #4259
...
write.table(fertilisedData, file = paste0(deFolder,"fungi2011Fertilised.tsv"), sep='\t', row.names=FALSE, col.names=FALSE)
